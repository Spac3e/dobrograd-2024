...
for i, v in ipairs({
	...
	{'spacer'},
	{'Наш дискорд', function() octoesc.OpenURL('https://discord.gg/4fG6bKY99P') end},
	{'spacer'},
	{'Главное меню', function() hangClosed() gui.ActivateGameUI() end},
	{'Отключиться', function() IGS.UI() end},
	{'Отключиться', function()
		local eID = LocalPlayer():EntIndex()
		Derma_Query('Может-быть вы хотите сделать пожертвование для нашего сервера?', 'Вы хотите задонатить?', 'Конечно!', function()
			IGS.UI()
		end, 'Воzможно частично', function()
	...
	end, function(pnl) pnl:SetAlpha(10) end},
}) do
	if v[1] == 'spacer' then
