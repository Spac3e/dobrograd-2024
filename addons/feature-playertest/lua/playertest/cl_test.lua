...
local servers = {
	default = {
		name = 'Доберман РолевойОпыт',
		desc = [[Самый ахуенный сервер по тематике Доброграда. Тут вы получите незабываемые ощущения гильзы во всех частях тела, а может и сами подарите эти ощущения близким.

У нас вы можете показать кто здесь главный, получил перманентный бан от менеджера проекта, либо создать своё ОПГ ЧОП ЧВК и захватить сервер (по рп конечно). НонРП - бан]],
	},
}
...
function test.welcomeScreen(attempts, hint, showMsg)
	...
	local links = octolib.button(bp, 'Ссылки', function()
		local m = DermaMenu()
		m:AddOption("Наш дискорд", function() octoesc.OpenURL('https://discord.gg/4fG6bKY99P') end)
		m:Open()
	end)
	links:SetWide(115)
	links:Dock(LEFT)
