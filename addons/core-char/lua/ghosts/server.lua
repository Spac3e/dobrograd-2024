...
local baseTime = octodeath.config.spawnTime
local minTime = octodeath.config.spawnTime
local maxTime = octodeath.config.spawnTime
...
function PM:CreateRagdoll(attacker, dmg)
	...
	ent:SetNetVar('Corpse.time', 'pohuy') -- stormfox zalupen CWI.TimeToString
...
hook.Add('PlayerSpawn', 'GhostSpawn', function(ply)
	...
	if ply:GetNetVar('Ghost') and (ply:GetNetVar('_SpawnTime', 0) > CurTime()) then
