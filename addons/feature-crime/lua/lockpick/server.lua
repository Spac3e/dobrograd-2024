...
function ply:Lockpick(ent, data)
	...
	local _data = {
		num = data.numOverride or ent.lockNum or baseLockPickPreset.num,
		time = data.timeOverride or ent.lockTime or baseLockPickPreset.time,
		space = data.spaceOverride or ent.lockSpace or baseLockPickPreset.space,
		width = data.widthOverride or ent.lockWidth or baseLockPickPreset.width,
	}

	self.lockpickCache.doorData = _data
	hook.Run('onLockpickStarted', self)
	self:SetNetVar('currentAction', L.lockpick)
	netstream.Start(self, 'dbg-lockpick', ent, key, _data)
end
